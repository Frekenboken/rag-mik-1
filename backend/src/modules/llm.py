...
def context_prompt(history, context, query_text):
    ...
    Используя только предоставленный контекст, ответь только на вопрос пользователя.
    Если в представленном контексте нет нужной информации, отвечай что в базе данных нет информации по этой теме.
