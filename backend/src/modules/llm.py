...
def context_prompt(history, context, query_text):
    ...
    Используя только предоставленный контекст, ответь на вопрос пользователя конкретно и кратко.
    Если в представленном контексте нет нужной информации, отвечай что в базе данных нет информации по этой теме и не давай никаких советов
